...
% ---
\begin{siglas}
	\item[ADAM] Adaptive Moment Estimation (Momento Estocástico Adaptativo)
	\item[AP] Average Precision (Precisão Média)
	\item[COCO] Common Objects in Context (Objetos Comuns no Contexto)
	\item[FP] Falso Positivo
	\item[FN] Falso Negativo   
	\item[GD] Gradiente Descendente
	\item[GRVA] Grupo de Estudos de Realidade Virtual e Aumentada 
	\item[GUI] Graphical User Interface (Interface Gráfica do Usuário)
	\item[IA] Inteligência Artificial 
	\item[IoU] Intersection Over the Union (Interseção sobre a União)
	\item[mAP] Mean-average Precision (Precisão Média)
	\item[NA] Neurônio Artificial
	\item[RNC] Rede Neural Convolucional
	\item[RV] Realidade Virtual
	\item[SGD] Stochastic Gradient Descent (Gradiente Estocástico Descendente)
	\item[VANT] Veículo Aéreo Não Tripulado	  	
	\item[VP] Verdadeiro Positivo
	\item[YOLO] You Look Only Once (Você Olha Apenas Uma Vez)
\end{siglas}
