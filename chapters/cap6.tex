...
\begin{figure}[!h]
    ...
    \label{fig:yolov5_box_loss}
    \end{minipage}
\end{figure}

Na Figura \ref{fig:yolov5_box_loss}, há apresentação dos resultados gerados automaticamente a partir do treinamento com a YOLOv5. No geral, todas as versões aqui apresentadas geram as mesmas dez métricas apresentadas. Aquelas com \textit{train} em seu nome, indicam que foram aferidas durante o processo de treinamento. Dentre elas, foi apresentada a \textit{train\textunderscore{}boxloss}, que calcula o erro médio associado a predição dos objetos com relação às marcações realizadas nas fotos. No geral, quanto maior o decréscimo, melhor a precisão da rede. De maneira semelhante, \textit{train obj\textunderscore{}loss} avalia o erro  na capacidade da rede em detectar o que é de fato um objeto a ser encontrado, e o que é apenas \textit{background}, ou seja, tudo aquilo que não é relevante para a detectação. Nesse caso, a redução desse valor ao longo do treinamento, indica uma boa precisão. A métrica \textit{train cls\textunderscore{}loss}, avalia quão bem a rede consegue diferenciar classes diferentes treinadas em conjunto. No caso, como foi treinado apenas uma classe, esse valor ou é constante ou nulo. Tanto \textit{metrics precision}, \textit{metrics recall}, \textit{metrics mAP\textunderscore{}0.5} e \textit{metrics mAP\textunderscore{}0.5\textemdash 0.95} foram explicadas anteriormente. O acréscimo suave dessas métricas, indicam boa capacidade de detecção da rede, fato evidenciado já neste primeiro treinamento. Da mesma forma, mAP\textunderscore{}0.5 Por fim, as métricas com \textit{val} em seu nome foram aferidas durante o processo de validação, que ocorre ao fim de cada época. O comportamento desejado é o mesmo para as outras métricas: buscando redução do erro sempre, fato que também é evidenciado neste treinamento \cite{yolov8_box_loss}.
...
Para avaliar a eficácia do modelo na prática, foram realizados testes com um conjunto de 80 imagens não vistas anteriormente. A Tabela \ref{tab:yolov8-eval} resume o desempenho do modelo YOLOv8 com a configuração ideal (\textit{SGD} e batch size 16), revelando que 72,34375\% das imagens foram corretamente identificadas, refletindo uma taxa de acerto considerável. Conforme trabalho de \citeonline{barbado2022aplicaccao}, essa uma taxa de acerto para uma RNC acima de 70\%, para uma aplicação em nível de protótipo, mostra-se demasiada satisfatória. Com isso, o propósito do trabalho, que se coloca como uma proposição de utilização de uma tecnologia em evidência para melhora de um processo, é dado como atingido. 
...
\begin{table}[!hbt]
    ...
    \cellcolor[gray]{0.9} \begin{sideways} \textbf{Treinamento utilizando YOLO} \end{sideways} & 
