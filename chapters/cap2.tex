...
As RNC representam por si só uma revolução no campo da visão computacional, tornando-se a base para uma variedade de aplicações, desde reconhecimento de imagens até análise de vídeos. Entre as arquiteturas mais notáveis estão a LeNet, AlexNet, VGGNet, GoogLeNet e ResNet, cada uma contribuindo com avanços significativos em profundidade, eficiência e precisão. Além dessas, uma destaca-se em especial: a YOLO (\textit{“You Only Look Once”}), uma abordagem inovadora para treinamento e detecção de objetos \cite{carboni2021sistema}.

\subsection{You Look Only Once}
\label{sec:yolo}

YOLO, uma Rede Neural Convolutiva (RNC), é conhecida por sua eficácia e precisão na detecção de objetos em imagens e vídeos. Desenvolvida por Joseph Redmon, Santosh Divvala, Ross Girshick e Ali Farhadi em 2015, a arquitetura do YOLO foi apresentada em sua primeira versão no artigo \textit{"You Only Look Once: Unified, Real-Time Object Detection"} \cite{redmon2016youlookonce}.
\begin{figure}[!h]
    \center
    \begin{minipage}{1\linewidth}
    \center
...
Com esses valores, pode-se calcular os resultados da saída que o treinamento da rede YOLO fornece. Na Equação (1), tem-se o cálculo da Recall, que calcula a capacidade da rede de detectar todos os objetos relevantes em uma imagem. Já a Precisão, definida na Equação (2), refere-se à capacidade da rede de encontrar apenas resultados relevantes. 
