...
\begin{figure}[!h]
    ...
    \includegraphics[width=0.9\linewidth]{img/cap2/substation.png}
    \caption{Representação adaptada dos componentes do sistema de geração e transmissão de energia de \cite{lopes2012smart}}.
...
\begin{figure}[!h]
    ...
    \caption{Representação de um NA adaptado de \cite{adorno2017}}.
    \label{fig:na}
    \end{minipage}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{Potencial de ativação}: É o resultado decorrente da discrepância entre o valor gerado pelo combinador linear e o limiar de ativação. Se esse resultado for positivo, indicando \(u \geq 0\), o neurônio gera um potencial de excitação; caso contrário, o potencial é inibitório.
...
A fim de avaliar o desempenho de um treinamento na arquitetura YOLO, é preciso entender os resultados fornecidos pelo modelo. De acordo com \citeonline{padilla2020survey}, basicamente a YOLO utiliza a métrica chamada \textit{Average Precision} (AP) (“Precisão Média”, em tradução livre). Ela se baseia no conceito de IoU (“\textit{Intersection over the Union}”, Intersecção sobre a União, em tradução livre), que calcula uma razão entre a interseção da detecção feita pelo algoritmo com relação à marcação (\textit{bounding box}) realizada em cima da área dividida pela união dessas duas áreas (Figura \ref{fig:iou}). Essa razão é comparada com um valor pré-estabelecido, o \textit{thresholds}, que é referido por L. A partir desse valor, é possível que no processo de detecção retorne três diferentes resultados na pesquisa pela classe desejada. São eles: Verdadeiro Positivo (VP), Falso Positivo (FP) e Falso Negativo (FN). VP trata-se dos resultados considerados corretos que a rede neural retorna, que seriam todos resultados que IoU > L. FP já seriam os resultados em que IoU < L, que são tidos como incorretos. FN, por sua vez, trata dos resultados totalmente fora do esperado. O valor de L pode ser entendido como uma tolerância para os resultados encontrados. Quando o valor da área tem de ser pelo menos 50 por cento da intersecção das duas áreas, encontra-se a métrica mAP50. Por sua vez, quando o valor é acima de 95, obtêm-se mAP95. Por fim, se deseja uma média entre essas duas métricas, é calculado portanto mAP50-95.
...
\begin{figure}[!h]
    ...
    \caption{ Cálculo de IoU, adaptação de . \cite{padilla2020survey}}
    \label{fig:iou}
    \end{minipage}
\end{figure}
